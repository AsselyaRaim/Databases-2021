...
SELECT p.category_id, c.name, sum(oi.quantity) as total_sales FROM
category c INNER JOIN products p on c.id = p.category_id INNER JOIN order_items oi on p.id = oi.product_id
GROUP BY p.category_id, c.name
ORDER BY total_sales DESC LIMIT 10;

--Top 10 popular super-categories
SELECT c.super_category, sum(oi.quantity) as total_sales FROM
category c INNER JOIN products p on c.id = p.category_id INNER JOIN order_items oi on p.id = oi.product_id
GROUP BY c.super_category
ORDER BY total_sales DESC LIMIT 10;
